...
export default function SingleInfluencerUser ({ history, canEdit = false }) {
  ...
  const updateUserStatus = async (payload = {}) => {
    setLoading(true)
    const { data } = await request.post('/api/admin/users/status', {
      ...payload,
      user_id: user.id
    })
    setUser(data)
          ...
          <Card>
            ...
            <CardBody>
              {!user?.userSocialAccounts?.length ? (
                ...
                <div className='users-page-view-table text-truncate'>
                  {user.userSocialAccounts.map(account => {
                    return (
                      <div key={account.id} className='d-flex user-info'>
                        <div className='user-info-title font-weight-bold'>
                          {startCase(account.platform)}
                        </div>
                        <a
                          target='_blank'
                          rel='noopener noreferrer'
                          href={account.external_link}
                        >
                          <div className='text-truncate'>
